...
/**
 * Mostly a copy of org.eclipse.nebula.widgets.grid.internal.DefaultCellRender,
 * with the addition of enabling to define TextStyles and apply them to the
 * cell.
 */
@SuppressWarnings("all")
public class TextStyleCellRenderer extends GridCellRenderer {
	...
	private final Consumer<TextLayout> textLayoutConsumer;

	public TextStyleCellRenderer(final Consumer<TextLayout> textLayoutConsumer) {
		this.textLayoutConsumer = textLayoutConsumer;
	}
	...
	public void paint(final GC gc, final Object value) {
		...
		if (drawAsSelected) {
			...
			final Color foreground = item.getForeground(getColumn());
		...
		if (!isWordWrap()) {
			...
			if (textLayoutConsumer != null) {
				textLayoutConsumer.accept(textLayout);
			}
	...
	public boolean notify(final int event, final Point point, final Object value) {
		...
		if (isCheck()) {
			if ((event == IInternalWidget.MouseMove) && overCheck(item, point)) {
				setHoverDetail("check");
				return true;
			}

			if ((event == IInternalWidget.LeftMouseButtonDown) && overCheck(item, point)) {
		...
		if (isTree() && item.hasChildren()) {
			if ((event == IInternalWidget.MouseMove) && overToggle(item, point)) {
				setHoverDetail("toggle");
				return true;
			}

			if ((event == IInternalWidget.LeftMouseButtonDown) && overToggle(item, point)) {
