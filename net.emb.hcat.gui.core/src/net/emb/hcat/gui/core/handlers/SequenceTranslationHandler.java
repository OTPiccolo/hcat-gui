...
import net.emb.hcat.gui.core.wizards.SequenceTranslationWizard;

/**
 * Opens a wizard to translate sequences.
 *
 * @author OT Piccolo
 */
public class SequenceTranslationHandler {
	...
	@SuppressWarnings("javadoc")
	@Execute
	public void execute(final EPartService partService, final Shell shell, final SequenceTranslationWizard wizard) {
	...
	private void createPart(final EPartService partService, final SequenceTranslationWizard wizard, final MPart orgPart) {
		final String name = orgPart.getLabel();
		final String label = name + " (" + wizard.getTransformationData().name + ")"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	private WizardDialog makeWizardDialog(final Shell shell, final SequenceTranslationWizard wizard, final Collection<Sequence> sequences) {
